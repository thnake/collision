...
GLvoid glmDrawMesh(GLMmodel* model, GLfloat *triVerts)
{
	GLuint i, j;
	GLMgroup* group;
	GLMtriangle* triangle;
	...
	while (group) {
		...
		for (i = 0; i < group->numtriangles; i++) {
			...
			for (j=0; j<3; j++) {
				...
				//punkt speichern 
				vertex = &model->vertices[3 * triangle->vindices[j]];
				triVerts[i*9 + j*3 + 0] = vertex[0];
				triVerts[i*9 + j*3 + 1] = vertex[1];
				triVerts[i*9 + j*3 + 2] = vertex[2];
